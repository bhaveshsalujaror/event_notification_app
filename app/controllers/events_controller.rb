require 'webmock'

class EventsController < ApplicationController
  include WebMock::API
  WebMock.enable!

  before_action :authenticate_user!

  ITERABLE_EVENTS_TRACK_URL = "https://api.iterable.com/api/events/track".freeze
  ITERABLE_EMAIL_TARGET_URL = "https://api.iterable.com/api/email/target".freeze

  # Action to display all events
  def index
    @events = Event.all
    @event_a = Event.new
    @event_b = Event.new
  end

  # Action to create Event A
  def event_a
    @user = current_user
    @event = @user.events.build(event_params)

    if @event.save
      flash[:notice] = "Event A Created Successfully"
      send_event_to_iterable(@event.title, @user)
    else
      flash[:alert] = "Event A Creation Unsuccessful"
    end
    
    redirect_to root_path
  end

  # Action to create Event B
  def event_b
    @user = current_user
    @event = @user.events.build(event_params)

    if @event.save
      flash[:notice] = "Event B Created Successfully"
      send_event_to_iterable(@event.title, @user)
      send_email_notification(@user, @event)
      flash[:alert] = "Email For Event B Sent Successfully to #{@user.email}"
    else
      flash[:alert] = "Event B Creation Unsuccessful"
    end
    
    redirect_to root_path
  end

  private

  # Strong parameters for event creation
  def event_params
    params.require(:event).permit(:title, :description)
  end

  # Method to send event data to Iterable
  def send_event_to_iterable(event_name, user)
    event_and_user_detail = { event_name: event_name, user_email: user.email }

    # Stubbing the request for testing purposes
    stub_request(:post, ITERABLE_EVENTS_TRACK_URL).
      with(headers: { 'Accept'=>'*/*', 'User-Agent'=>'Ruby' }).
      to_return(status: 200, body: event_and_user_detail.to_json, headers: {})

    # Sending the event data to Iterable
    HTTParty.post(ITERABLE_EVENTS_TRACK_URL)
  end

  # Method to send email notification using Iterable
  def send_email_notification(user, event)
    email_data = {
      campaignId: event.id,
      recipientEmail: user.email,
      recipientUserId: user.id, # Assuming user.id is the user's Iterable user ID
      dataFields: { event_name: event.title, event_description: event.description },
      sendAt: DateTime.now, # Optional send time
      allowRepeatMarketingSends: true,
      metadata: {}
    }

    # Stubbing the request for testing purposes
    stub_request(:post, ITERABLE_EMAIL_TARGET_URL).
      with(headers: { 'Accept'=>'*/*', 'User-Agent'=>'Ruby' }).
      to_return(status: 200, body: { msg: "Email Sent", code: "Success", params: email_data }.to_json, headers: {})

    # Sending the email data to Iterable
    HTTParty.post(ITERABLE_EMAIL_TARGET_URL, body: email_data.to_json, headers: { 'Content-Type' => 'application/json' })

    # Handle the response if needed
  end
end
